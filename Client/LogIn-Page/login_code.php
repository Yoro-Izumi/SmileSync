...
function increment_attempts($db_connection, $user_id) {
    ...
    if ($attempt) {
        $stmtUpdate = mysqli_prepare($db_connection, "UPDATE smilesync_patient_attempts SET patient_number_of_attempts = patient_number_of_attempts + 1, patient_last_attempt_time = NOW() WHERE patient_account_id = ?");
        mysqli_stmt_bind_param($stmtUpdate, 'i', $user_id);
        mysqli_stmt_execute($stmtUpdate);
    } else {
        $stmtInsert = mysqli_prepare($db_connection, "INSERT INTO smilesync_patient_attempts (patient_attempts_id,patient_account_id, patient_number_of_attempts, patient_first_attempt_time, patient_last_attempt_time) VALUES (NULL,?, 1, NOW(), NOW())");
