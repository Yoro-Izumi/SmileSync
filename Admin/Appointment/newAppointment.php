...
nextButton.id = 'nextNewBtn'; // Start with a different ID for "Next"

// Update button behavior based on text content
const updateButtonId = () => {
  if (nextButton.textContent.trim() === 'Submit') {
    nextButton.id = 'submitNewBtn';
  } else {
    nextButton.id = 'nextNewBtn';
  }
};
...
nextButton.addEventListener('click', () => {
  ...
  if (currentStep === formSections.length - 1) {
    nextButton.textContent = 'Submit'; // Update button text to "Submit"
  }
  updateButtonId(); // Ensure correct ID after text change
});
...
prevButton.addEventListener('click', () => {
  ...
  updateButtonId(); // Ensure correct ID after text reset
});
