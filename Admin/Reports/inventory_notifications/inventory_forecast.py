...
def process_notification(cursor, item_name, item_id, date, stock_level, threshold):
    """
    Decision tree for generating notifications and inserting into database.
    """
    if stock_level <= 0:
        notification_message = f"Item {item_name} (ID: {item_id}) will be out of stock on {date.strftime('%Y-%m-%d')}!"
        insert_notification(cursor, notification_message, date)
    elif stock_level <= threshold:
        notification_message = f"Warning: Item {item_name} (ID: {item_id}) stock is low ({stock_level}) on {date.strftime('%Y-%m-%d')}!"
        insert_notification(cursor, notification_message, date)

def insert_notification(cursor, message, date):
    """
    Inserts a notification message into the admin and super admin notification tables.
    Avoids duplicate notifications.
    """
    notification_date = date.strftime('%Y-%m-%d')
    
    # Check for duplicates in both tables
    check_query = """
        SELECT COUNT(*) FROM smilesync_admin_notifications 
        WHERE notification = %s AND notification_date = %s
    """
    cursor.execute(check_query, (message, notification_date))
    if cursor.fetchone()[0] == 0:
        ...
        cursor.execute(
            ...
            (notification_id, notification, super_admin_id, notification_date) 
            VALUES (NULL, %s, NULL, %s)
            """,
            (message, notification_date)
        )

# Step 4: Group data by item_id
threshold = 10  # Define a threshold for low stock warning
...
for item_id, group in data.groupby('item_id'):
    try:
        ...
        # Step 5: Process notifications using the decision tree
        for date, value in zip(forecast_dates, forecast):
            process_notification(cursor, group['item_name'].iloc[0], item_id, date, value, threshold)

        # Store results
        for date, value in zip(forecast_dates, forecast):
            forecast_results.append([item_id, group['item_name'].iloc[0], date.strftime('%Y-%m-%d'), value])
...
# Step 7: Commit database changes and close connection
try:
