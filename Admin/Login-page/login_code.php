...
function handle_login_attempts($connect_db, $user_id, $user_type, $password, $stored_password) {
    ...
    if ($user_type === 'admin') {
        $stmtCheckAttempts = mysqli_prepare($connect_db, "SELECT * FROM admin_login_attempts WHERE admin_id = ?");
        ...
        $stmtCheckAttempts = mysqli_prepare($connect_db, "SELECT * FROM super_admin_login_attempts WHERE super_admin_id = ?");
        mysqli_stmt_bind_param($stmtCheckAttempts, 'i', $user_id);
        mysqli_stmt_execute($stmtCheckAttempts);
        $resultCheckAttempts = mysqli_stmt_get_result($stmtCheckAttempts);
        $login_attempt = mysqli_fetch_assoc($resultCheckAttempts);
...
function reset_attempts($connect_db, $user_id, $user_type) {
    if ($user_type === 'admin') {
        $stmtReset = mysqli_prepare($connect_db, "DELETE FROM admin_login_attempts WHERE admin_id = ?");
    } else {
        $stmtReset = mysqli_prepare($connect_db, "DELETE FROM super_admin_login_attempts WHERE super_admin_id = ?");
    }
...
function increment_attempts($connect_db, $user_id, $user_type) {
    if ($user_type === 'admin') {
        $stmtCheck = mysqli_prepare($connect_db, "SELECT * FROM admin_login_attempts WHERE admin_id = ?");
    } else {
        $stmtCheck = mysqli_prepare($connect_db, "SELECT * FROM super_admin_login_attempts WHERE super_admin_id = ?");
    }
    ...
    if ($attempt) {
        if ($user_type === 'admin') {
            $stmtUpdate = mysqli_prepare($connect_db, "UPDATE admin_login_attempts SET attempts = attempts + 1, last_attempt_time = NOW() WHERE admin_id = ?");
        } else {
            $stmtUpdate = mysqli_prepare($connect_db, "UPDATE super_admin_login_attempts SET attempts = attempts + 1, last_attempt_time = NOW() WHERE super_admin_id = ?");
        }
        ...
        if ($user_type === 'admin') {
            $stmtInsert = mysqli_prepare($connect_db, "INSERT INTO admin_login_attempts (admin_id, attempts, first_attempt_time, last_attempt_time) VALUES (?, 1, NOW(), NOW())");

        } else {
            $stmtInsert = mysqli_prepare($connect_db, "INSERT INTO super_admin_login_attempts (super_admin_id, attempts, first_attempt_time, last_attempt_time) VALUES (?, 1, NOW(), NOW())");
        }
