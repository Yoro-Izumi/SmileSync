...
// Prepare an array to hold the dates for current day and the past six days
$dates = [];
for ($i = 0; $i < 7; $i++) {
    $dates[$i] = date('Y-m-d', strtotime("-$i days")); // Store the current day and past 6 days
}
...
foreach ($resultInventoryContentID as $inventoryContentIDRow) {
    ...
    // Initialize an array to hold remaining quantities for the past 7 days (current day + past 6 days)
    ...
    if ($inventoryContent = mysqli_fetch_assoc($resultInventoryContent)) {
        ...
        // Loop through each date (from current day to past 6 days)
        foreach ($dates as $index => $date) {
            // Query to get the usage for this item on this particular date, converting datetime to date using the DATE() function
            ...
            if ($index === 0) {
                // For the current day, subtract today's usage from the initial quantity
        ...
        echo "<tr>
                ...
                <td data-label='Date 1'>{$remainingQuantities[0]}</td> <!-- Current day -->
